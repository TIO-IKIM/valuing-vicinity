...
class MAFYclassRes18Net(BaseNet):
    """
        - structure to fit into MAF pipeline via split into encoder, decoder, segmentation_head
        - ResNet18 backbone
    ...
    def __init__(self, cfg):
        ...
        self.merge = MultiScaleMergeBlock(
            ...
            base_fm=self.base_fm,
            fm_delay=self.fm_delay,
            scale_side=cfg['scale_list'],
            base_main=cfg['patch_size'],
            act=self.get_act(self.act_type),
            norm=self.get_norm(self.norm_type),
            gpu=cfg['gpu']
        )
    ...
    def encoder(self, x_main, x_tail):
        # main path
        from_skip = []
